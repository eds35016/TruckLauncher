...
{
    // Extract truck pack info if present
    if (!m_extra_info.isEmpty()) {
        auto truck_pack_it = m_extra_info.constFind("TruckPack");
        if (truck_pack_it != m_extra_info.constEnd() && truck_pack_it.value() == "true") {
            auto pack_name_it = m_extra_info.constFind("TruckPackName");
            auto pack_version_it = m_extra_info.constFind("TruckPackVersion");
            
            if (pack_name_it != m_extra_info.constEnd() && pack_version_it != m_extra_info.constEnd()) {
                m_hasPendingTruckPackInfo = true;
                m_pendingTruckPackName = pack_name_it.value();
                m_pendingTruckPackVersion = pack_version_it.value();
                
                // Extract MaxMemAlloc if present
                auto max_mem_it = m_extra_info.constFind("MaxMemAlloc");
                if (max_mem_it != m_extra_info.constEnd()) {
                    bool ok;
                    m_pendingMaxMemAlloc = max_mem_it.value().toInt(&ok);
                    if (!ok) {
                        m_pendingMaxMemAlloc = -1;
                    }
                }
                
                qDebug() << "InstanceImportTask: Pending truck pack info -" << m_pendingTruckPackName << "v" << m_pendingTruckPackVersion << "RAM:" << m_pendingMaxMemAlloc << "MB";
            }
        }
    }
}
