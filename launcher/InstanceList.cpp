   ...
   private slots:
    void childSucceeded()
    {
        ...
        if (m_parent->commitStagedInstance(m_stagingPath, *m_child.get(), m_child->group(), *m_child.get())) {
            // Check if this is an InstanceImportTask with pending truck pack info
...
void InstanceList::applyTruckPackInfo(const QString& instanceName, const QString& packName, const QString& packVersion, int maxMemAlloc)
{
    ...
    for (auto inst : m_instances) {
        if (inst->name() == instanceName) {
            ...
            if (maxMemAlloc > 0) {
                qDebug() << "  Setting MaxMemAlloc to" << maxMemAlloc << "MB";
                inst->settings()->set("OverrideMemory", true);
                inst->settings()->set("MaxMemAlloc", maxMemAlloc);
            }
            
            inst->saveNow();
            qDebug() << "  Truck pack info applied and saved";
            return;
        }
    }
    
    qWarning() << "InstanceList::applyTruckPackInfo: Could not find instance with name:" << instanceName;
}
