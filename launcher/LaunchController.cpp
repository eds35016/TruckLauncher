...
#include "ui/dialogs/TruckPackUpdateDialog.h"
...
void LaunchController::executeTask()
{
    ...
    // Check if this is a truck pack and if it's outdated
    qDebug() << "LaunchController: Checking if instance is a truck pack...";
    bool isTruck = m_instance->isTruckPack();
    qDebug() << "LaunchController: isTruckPack() returned:" << isTruck;
    
    if (isTruck) {
        QString packName = m_instance->getTruckPackName();
        QString currentVersion = m_instance->getTruckPackVersion();
        qDebug() << "LaunchController: Truck pack detected - Name:" << packName << "Version:" << currentVersion;
        
        auto versionManager = APPLICATION->truckPackVersionManager();
        qDebug() << "LaunchController: Version manager exists:" << (versionManager != nullptr);
        
        if (versionManager && versionManager->isLoaded()) {
            qDebug() << "LaunchController: Version manager is loaded";
            QString latestVersion = versionManager->getLatestVersion(packName);
            qDebug() << "LaunchController: Latest version for" << packName << "is:" << latestVersion;

            if (!latestVersion.isEmpty() && currentVersion != latestVersion) {
                qDebug() << "LaunchController: Pack is outdated! Showing update dialog...";
                // Show update dialog
                TruckPackUpdateDialog dialog(packName, currentVersion, latestVersion, m_parentWidget);
                dialog.exec();

                switch (dialog.getResult()) {
                    case TruckPackUpdateDialog::UpdateNow:
                        {
                            qDebug() << "LaunchController: User chose to update the pack";
                            // Get the pack info to download the new version
                            TruckPackInfo packInfo = versionManager->getPackInfo(packName);
                            if (packInfo.packUrl.isEmpty()) {
                                emitFailed(tr("Failed to get download URL for pack update."));
                                return;
                            }
                            
                            qDebug() << "LaunchController: Starting pack update from" << packInfo.packUrl;
                            
                            // Signal that we want to update this pack
                            emit instanceUpdateRequested(m_instance->id(), packInfo.packUrl, packInfo.packName, packInfo.packVersion);
                            
                            // Abort the launch since we're updating instead
                            emitAborted();
                            return;
                        }
                    case TruckPackUpdateDialog::LaunchAnyway:
                        // Continue with launch
                        break;
                    case TruckPackUpdateDialog::Cancel:
                    default:
                        emitAborted();
                        return;
                }
            } else {
                qDebug() << "LaunchController: Pack is up to date or version check failed";
            }
        } else {
            qDebug() << "LaunchController: Version manager not loaded yet";
        }
    }
