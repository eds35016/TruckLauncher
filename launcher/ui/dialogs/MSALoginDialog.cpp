...
void MSALoginDialog::authorizeWithBrowserWithExtra(QString url, QString code, int expiresIn)
{
    ...
    if (isDefaultUrl) {
        ui->qrMessage->setText(tr("Open %1 or scan the QR and enter the code below.").arg(linkString));
    } else {
        ui->qrMessage->setText(tr("Open %1 and enter the code below.").arg(linkString));
    }
}
